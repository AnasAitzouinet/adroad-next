...
export async function getServerSideProps({ req, res }) {
  try {
    ...
    // Pass the user data as props to the component
    return {
      props: {
        userData,
      },
    ...
    console.error(error);
    // Redirect to login page if there is an error with the token
    return {
      redirect: {
        destination: '/login',
        permanent: false,
...
export default function Dashborad({ userData }) {
                  ...
                  <div>
                    <Menu.Button className="flex max-w-xs items-center rounded-full bg-white text-sm  lg:rounded-md lg:p-2 lg:hover:bg-gray-50">
                      ...
                      <span className="ml-3 hidden text-sm font-medium text-gray-700 lg:block">
                        <span className="sr-only">Open user menu for </span>{userData.userName}
                      </span>
